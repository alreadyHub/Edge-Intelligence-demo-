...
const locationColors: Record<string, string> = {
  'back-bay': 'bg-violet-600/20 text-violet-600 dark:text-violet-300',
  'south-end': 'bg-emerald-400/10 text-emerald-600 dark:text-emerald-400',
  'cambridge': 'bg-amber-400/10 text-amber-600 dark:text-amber-400',
}
  ...
  return (
    <div className={`rounded-lg border p-4 ${
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <button onClick={() => setExpanded(!expanded)} className="text-zinc-400 dark:text-zinc-500 hover:text-zinc-600 dark:hover:text-zinc-300">
      ...
      {expanded && messages.length > 0 && (
        <div className="space-y-2 pt-3 border-t border-gray-200 dark:border-zinc-800/50">
    ...
    <div className="bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-lg px-3 py-2 text-xs shadow-xl">
      ...
      {payload.map((p) => (
        <div key={p.name} className="flex items-center gap-2">
          ...
          <span className="font-mono text-zinc-800 dark:text-zinc-100">{p.value?.toFixed(1)}</span>
        </div>
      ))}
    </div>
  )
...
export default function EmployeeDetailPage({ params }: { params: { id: string } }) {
    ...
    return (
      <>
        ...
        <div className="p-6 blur-sm opacity-30 pointer-events-none">
          <div className="h-32 bg-gray-100 dark:bg-zinc-900 rounded-xl border border-gray-200 dark:border-zinc-800" />
        </div>
      </>
    )
  }
  ...
  const avatarColor = locationColors[employee.locationId] || 'bg-gray-100 dark:bg-zinc-700 text-zinc-600 dark:text-zinc-300'
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <Link href="/employees" className="flex items-center gap-1.5 text-xs text-zinc-400 dark:text-zinc-500 hover:text-zinc-600 dark:hover:text-zinc-300 mb-4 transition-colors">
      ...
      <div className="flex items-start justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <span className="text-sm text-zinc-500 dark:text-zinc-400">{employee.role}</span>
        ...
        <div className="text-right">
          ...
          <p className="text-xs text-zinc-400 dark:text-zinc-600 mt-1">{employee.feedbackCounts[scoreKey]} responses</p>
        </div>
      </div>

      {/* Performance timeline */}
      <div className="rounded-xl border border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5 mb-6">
        <h2 className="text-sm font-semibold text-zinc-800 dark:text-zinc-100 mb-4">Performance Timeline</h2>
        <div className="h-52">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData} margin={{ top: 5, right: 10, left: -20, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke={gridColor} />
              <XAxis dataKey="week" tick={{ fill: tickColor, fontSize: 11 }} axisLine={{ stroke: gridColor }} tickLine={false} />
              <YAxis domain={[2, 5]} tick={{ fill: tickColor, fontSize: 11 }} axisLine={false} tickLine={false} />
        ...
        <p className="text-xs text-zinc-400 dark:text-zinc-600 mt-1">Dashed line = {employee.locationName} location average</p>
      </div>

      {/* What members are saying */}
      <div className="mb-6">
        <h2 className="text-sm font-semibold text-zinc-800 dark:text-zinc-100 mb-3">What Members Are Saying</h2>
        ...
        {/* All SMS examples */}
        <div className="mt-4">
          <h3 className="text-xs text-zinc-400 dark:text-zinc-500 font-medium mb-2 uppercase tracking-wider">Member Feedback</h3>
      ...
      <div className="rounded-xl border border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-semibold text-zinc-800 dark:text-zinc-100">Coaching Notes</h2>
          <button
        ...
        {editingNotes ? (
          <textarea
            ...
            className="w-full bg-gray-50 dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-lg p-3 text-sm text-zinc-700 dark:text-zinc-300 resize-none focus:outline-none focus:ring-1 focus:ring-violet-500"
            rows={4}
          />
        ) : (
