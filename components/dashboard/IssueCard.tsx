...
export function IssueCard({ issue, rank, maxFrequency, locked }: IssueCardProps) {
  ...
  if (locked) {
    return (
      <div className="relative rounded-lg border border-zinc-800 p-3 overflow-hidden">
        <div className="blur-sm opacity-40 pointer-events-none">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-zinc-300">Issue category</span>
            <span className="font-mono text-xs text-zinc-500">— mentions</span>
          </div>
          <div className="h-1.5 bg-zinc-800 rounded-full w-3/4" />
        </div>
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="flex items-center gap-2 bg-zinc-900/90 rounded-md px-3 py-1.5 border border-zinc-700">
            <Lock className="w-3 h-3 text-zinc-500" />
            <span className="text-xs text-zinc-400">Unlock in Premium</span>
          </div>
        </div>
      </div>
    )
      ...
      >
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-zinc-200">{issue.name}</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="font-mono text-xs text-zinc-400">{issue.frequency} mentions</span>
            {expanded ? (
