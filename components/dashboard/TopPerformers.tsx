    ...
    <div className="grid grid-cols-2 gap-4">
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {topThree.map((emp) => (
            <Link key={emp.id} href={`/employees/${emp.id}`} className="flex items-center gap-3 p-2 rounded-md hover:bg-gray-100 dark:hover:bg-zinc-800 transition-colors">
              <div className="w-8 h-8 rounded-full bg-violet-600/20 flex items-center justify-center text-xs font-semibold text-violet-600 dark:text-violet-300">
                {emp.name.charAt(0)}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm text-zinc-800 dark:text-zinc-100 font-medium truncate">{emp.name}</p>
                <p className="text-xs text-zinc-500 dark:text-zinc-500">{emp.locationName}</p>
              </div>
